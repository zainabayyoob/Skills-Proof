...
import { ShieldCheck, Sparkles, RotateCcw, Award } from 'lucide-react';
import { RoleSwitcher } from './RoleSwitcher';

export const Navbar = ({ currentRole, onSwitchRole, onResetData, student }) => {
  return (
    <header className="sticky top-0 z-40 bg-slate-950/90 backdrop-blur-md border-b border-slate-800/80 px-4 lg:px-8 py-3 transition-colors">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-4">
        ...
        {/* Center/Right: Role Switcher & Reset Button */}
        <div className="flex items-center gap-3">
          ...
          {/* Active Profile Info -> Clickable to /profile */}
          <Link
            to="/profile"
            title="View & Edit Student Profile"
            className="flex items-center gap-2.5 pl-2 border-l border-slate-800 hover:opacity-85 transition-opacity group cursor-pointer"
          >
            <img
              src={
                currentRole === 'STUDENT'
                  ? student?.avatarUrl || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=100"
            ...
            <div className="hidden lg:block text-left">
              <p className="text-xs font-bold text-slate-200 leading-tight group-hover:text-brand-300 transition-colors">
                {currentRole === 'STUDENT'
              ...
              <p className={`text-[10px] font-semibold flex items-center gap-1 ${
                currentRole === 'STUDENT' && (!student?.verifiedSkills || student?.verifiedSkills?.length === 0)
                  ? 'text-amber-400'
                  : 'text-emerald-400'
              }`}>
                ...
                {currentRole === 'STUDENT'
                  ? student?.verifiedSkills?.length > 0
                    ? `Readiness: ${student.careerReadiness}%`
                    : 'Unverified (0%)'
                  : 'Verified Entity'}
              </p>
            </div>
